...
// result is preserved.
func NullsafeAdd(v1, v2 sqltypes.Value, resultType proto.Type) sqltypes.Value {
	if v1.IsNull() {
...
func NullsafeHashcode(v sqltypes.Value) (int64, error) {
	...
	return 0, fmt.Errorf("types does not support hashcode yet: %v", v.Type())
}
...
func addNumericWithError(v1, v2 EvalResult) (EvalResult, error) {
	...
	return EvalResult{}, fmt.Errorf("invalid arithmetic between: %s %s", v1.Value().String(), v2.Value().String())
...
func intPlusIntWithError(v1, v2 int64) (EvalResult, error) {
	result := v1 + v2
	if (result > v1) != (v2 > 0) {
		return EvalResult{}, fmt.Errorf("BIGINT value is out of range in %v + %v", v1, v2)
	}
...
func intMinusUintWithError(v1 int64, v2 uint64) (EvalResult, error) {
	if v1 < 0 || v1 < int64(v2) {
		return EvalResult{}, fmt.Errorf("BIGINT UNSIGNED value is out of range in %v - %v", v1, v2)
	}
...
func floatDivideAnyWithError(v1 float64, v2 EvalResult) (EvalResult, error) {
	...
	if divisorLessThanOne && resultMismatch {
		return EvalResult{}, fmt.Errorf("BIGINT is out of range in %v / %v", v1, v2.fval)
	}
