...
func Cast(v sqltypes.Value, typ proto.Type) (sqltypes.Value, error) {
	...
	if v.Type() == sqltypes.Expression {
		return sqltypes.NULL, fmt.Errorf("%v cannot be cast to %v", v, typ)
	}
...
func ToNative2(v sqltypes.Value) (interface{}, error) {
	var out interface{}
	var err error
	switch {
	case v.Type() == sqltypes.Null:
...
func (v EvalResult) toSQLValue(resultType proto.Type) sqltypes.Value {
	switch {
