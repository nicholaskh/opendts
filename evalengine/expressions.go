...
type (
	...
	Expr interface {
		...
		Type(env ExpressionEnv) (proto.Type, error)
		String() string
	}
	...
	BinaryExpr interface {
		...
		Type(left proto.Type) proto.Type
		String() string
	}
...
func (b *BindVariable) Evaluate(env ExpressionEnv) (EvalResult, error) {
	...
	if !ok {
		return EvalResult{}, errors.New("Bind variable not found")
	}
...
func (a *Addition) Type(left proto.Type) proto.Type {
	return left
}

//Type implements the BinaryExpr interface
func (m *Multiplication) Type(left proto.Type) proto.Type {
	return left
}

//Type implements the BinaryExpr interface
func (d *Division) Type(proto.Type) proto.Type {
	return sqltypes.Float64
}

//Type implements the BinaryExpr interface
func (s *Subtraction) Type(left proto.Type) proto.Type {
	return left
}

//Type implements the Expr interface
func (b *BinaryOp) Type(env ExpressionEnv) (proto.Type, error) {
...
//Type implements the Expr interface
func (b *BindVariable) Type(env ExpressionEnv) (proto.Type, error) {
	e := env.BindVars
	v, found := e[b.Key]
	if !found {
		return proto.Type_NULL_TYPE, fmt.Errorf("query arguments missing for %s", b.Key)
	}
	return v.Type, nil
}

//Type implements the Expr interface
func (l *Literal) Type(ExpressionEnv) (proto.Type, error) {
	return l.Val.typ, nil
}

//Type implements the Expr interface
func (c *Column) Type(ExpressionEnv) (proto.Type, error) {
...
func mergeNumericalTypes(ltype, rtype proto.Type) proto.Type {
	switch ltype {
	case sqltypes.Int64:
...
func evaluateByType(val *proto.BindVariable) (EvalResult, error) {
	switch val.Type {
	case sqltypes.Int64:
	...
	return EvalResult{}, fmt.Errorf("Type is not supported: %s", val.Type.String())
}

// debugString is
func (e *EvalResult) debugString() string {
	return fmt.Sprintf("(%s) %d %d %f %s", proto.Type_name[int32(e.typ)], e.ival, e.uval, e.fval, string(e.bytes))
}
