...
func (p *XFileLog) Init(config map[string]string) (err error) {
	...
	if !ok {
		err = fmt.Errorf("init XFileLog failed, not found format")
		return
	}
...
func (p *XFileLog) clean() (err error) {
	...
	for _, file := range files {
		...
		if filepath.Base(file) == fmt.Sprintf("%s.wf.log", p.filename) {
			continue
		}
...
func (p *XFileLog) rename(shuffix string) (err error) {
	...
	warnLog := p.path + "/" + p.filename + ".wf.log"
	newLog := fmt.Sprintf("%s/%s.log-%s.log", p.path, p.filename, shuffix)
	newWarnLog := fmt.Sprintf("%s/%s.wf.log-%s.wf.log", p.path, p.filename, shuffix)
...
// Error implements XLogger
func (p *XFileLog) Error(format string, a ...interface{}) error {
	if p.level > ErrorLevel {
		return nil
	}

	return p.errorx(XFileLogDefaultLogID, format, a...)
}

// Errorx implements XLogger
func (p *XFileLog) Errorx(logID, format string, a ...interface{}) error {
	if p.level > ErrorLevel {
		return nil
	}

	return p.errorx(logID, format, a...)
}
...
func (p *XFileLog) errorx(logID, format string, a ...interface{}) error {
	...
	fun, filename, lineno := getRuntimeInfo(p.skip)
	logText = formatLineInfo(p.runtime, fun, filepath.Base(filename), logText, lineno)
	return p.write(ErrorLevel, &logText, logID)
}
...
func (p *XFileLog) write(level int, msg *string, logID string) error {
	...
	if p.format == LogFormatPlain {
		...
		logJson, _ := json.Marshal(logTextLog)
