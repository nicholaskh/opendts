...
// Returns a SQLError.
func (c *Conn) readColumnDefinition(field *proto.Field, index int) error {
	...
	if field.ColumnLength != 0 || field.Charset != 0 {
		...
		// See doc.go.
		if IsNum(t) {
			field.Flags |= uint32(proto.MySqlFlag_NUM_FLAG)
		}
	}
...
// Returns a SQLError.
func (c *Conn) readColumnDefinitionType(field *proto.Field, index int) error {
	colDef, err := c.readEphemeralPacket()
	if err != nil {
		return NewSQLError(CRServerLost, SSUnknownSQLState, "%v", err)
	}
...
// Returns a SQLError.
func (c *Conn) parseRow(data []byte, fields []*proto.Field) ([]sqltypes.Value, error) {
...
func (c *Conn) parseComStmtExecute(prepareData map[uint32]*PrepareData, data []byte) (uint32, byte, error) {
	...
	for i := 0; i < len(prepare.ParamsType); i++ {
		...
		if (bitMap[i/8] & (1 << uint(i%8))) > 0 {
			...
			val, pos, ok = c.parseStmtArgs(payload, proto.Type(prepare.ParamsType[i]), pos)
		}
		if !ok {
...
func (c *Conn) parseStmtArgs(data []byte, typ proto.Type, pos int) (sqltypes.Value, int, bool) {
	switch typ {
	case sqltypes.Null:
...
func (c *Conn) writeColumnDefinition(field *proto.Field) error {
...
func (c *Conn) writeBinaryRow(fields []*proto.Field, row []sqltypes.Value) error {
	length := 0
