...
func NewMysqld(connParams *mysql.ConnParams) *Mysqld {
	result := &Mysqld{
	...
	result.dbaPool.Open(connParams)
...
func (mysqld *Mysqld) RunMysqlUpgrade() error {
	...
	if *socketFile != "" {
		log.Trace("executing Mysqld.RunMysqlUpgrade() remotely via mysqlctld server: %v", *socketFile)
	...
	// use dba credentials.
	defaultsFile, err := mysqld.defaultsExtraFile(mysqld.connParams)
	if err != nil {
		return err
	}
...
func (mysqld *Mysqld) Start(ctx context.Context, cnf *Mycnf, mysqldArgs ...string) error {
	// Execute as remote action on mysqlctld if requested.
	if *socketFile != "" {
		log.Trace("executing Mysqld.Start() remotely via mysqlctld server: %v", *socketFile)
		client, err := mysqlctlclient.New("unix", *socketFile)
		if err != nil {
			return fmt.Errorf("can't dial mysqlctld: %v", err)
		}
...
func (mysqld *Mysqld) startNoWait(ctx context.Context, cnf *Mycnf, mysqldArgs ...string) error {
	...
	case hook.HOOK_DOES_NOT_EXIST:
		...
		log.Trace("%v: No mysqld_start hook, running mysqld_safe directly", ts)
		vtMysqlRoot, err := vtenv.VtMysqlRoot()
		if err != nil {
			return err
		}
		...
		if err != nil {
			...
			log.Trace("%v: trying to launch mysqld instead", err)
		...
		go func() {
			...
			for scanner.Scan() {
				log.Trace("%v stderr: %v", ts, scanner.Text())
			}
		}()
		go func() {
			scanner := bufio.NewScanner(stdout)
			for scanner.Scan() {
				log.Trace("%v stdout: %v", ts, scanner.Text())
			}
		}()
		err = cmd.Start()
		if err != nil {
...
func (mysqld *Mysqld) Wait(ctx context.Context, cnf *Mycnf) error {
	return mysqld.wait(ctx, cnf, mysqld.connParams)
}
...
func (mysqld *Mysqld) wait(ctx context.Context, cnf *Mycnf, params *mysql.ConnParams) error {
	log.Trace("Waiting for mysqld socket file (%v) to be ready...", cnf.SocketFile)
	...
	for {
		...
		if statErr == nil {
			...
			log.Trace("mysqld socket file exists, but can't connect: %v", connErr)
...
func (mysqld *Mysqld) Shutdown(ctx context.Context, cnf *Mycnf, waitForMysqld bool) error {
	log.Trace("Mysqld.Shutdown")

	// Execute as remote action on mysqlctld if requested.
	if *socketFile != "" {
		log.Trace("executing Mysqld.Shutdown() remotely via mysqlctld server: %v", *socketFile)
		client, err := mysqlctlclient.New("unix", *socketFile)
		if err != nil {
			return fmt.Errorf("can't dial mysqlctld: %v", err)
		}
	...
	case hook.HOOK_DOES_NOT_EXIST:
		...
		log.Trace("No mysqld_shutdown hook, running mysqladmin directly")
		...
		cnf, err := mysqld.defaultsExtraFile(mysqld.connParams)
		if err != nil {
			return err
		}
		defer os.Remove(cnf)
	...
	// didn't start.
	if waitForMysqld {
		log.Trace("Mysqld.Shutdown: waiting for socket file (%v) and pid file (%v) to disappear",
...
func execCmd(name string, args, env []string, dir string, input io.Reader) (cmd *exec.Cmd, output string, err error) {
	...
	log.Trace("execCmd: %v %v %v", name, cmdPath, args)
	...
	output = string(out)
	if err != nil {
		log.Trace("execCmd: %v failed: %v", name, err)
		err = fmt.Errorf("%v: %v, output: %v", name, err, output)
	}
	log.Trace("execCmd: %v output: %v", name, output)
...
func (mysqld *Mysqld) installDataDir(cnf *Mycnf) error {
	...
	if mysqld.capabilities.hasInitializeInServer() {
		log.Trace("Installing data dir with mysqld --initialize-insecure")
		args := []string{
			"--defaults-file=" + cnf.path,
			"--basedir=" + mysqlBaseDir,
		...
		if _, _, err = execCmd(mysqldPath, args, nil, mysqlRoot, nil); err != nil {
			log.Warn("mysqld --initialize-insecure failed: %v\n%v", err, readTailOfMysqldErrorLog(cnf.ErrorLogPath))
			return err
		}
		return nil
	}

	log.Trace("Installing data dir with mysql_install_db")
	args := []string{
		"--defaults-file=" + cnf.path,
		"--basedir=" + mysqlBaseDir,
	}
	...
	if _, _, err = execCmd(cmdPath, args, nil, mysqlRoot, nil); err != nil {
		log.Warn("mysql_install_db failed: %v\n%v", err, readTailOfMysqldErrorLog(cnf.ErrorLogPath))
		return err
	}
	return nil
}
...
func (mysqld *Mysqld) initConfig(cnf *Mycnf, outFile string) error {
	...
	case hook.HOOK_DOES_NOT_EXIST:
		log.Trace("make_mycnf hook doesn't exist, reading template files")
...
func (mysqld *Mysqld) getMycnfTemplate() string {
	...
	if err != nil {
		log.Trace("this version of Vitess does not include built-in support for %v %v", mysqld.capabilities.flavor, mysqld.capabilities.version)
	}
	myTemplateSource.Write(b)
	...
	if extraCnf := os.Getenv("EXTRA_MY_CNF"); extraCnf != "" {
		...
		for _, path := range parts {
			...
			if dataErr != nil {
				log.Trace("could not open config file for mycnf: %v", path)
				continue
			}
...
func (mysqld *Mysqld) RefreshConfig(ctx context.Context, cnf *Mycnf) error {
	// Execute as remote action on mysqlctld if requested.
	if *socketFile != "" {
		log.Trace("executing Mysqld.RefreshConfig() remotely via mysqlctld server: %v", *socketFile)
		client, err := mysqlctlclient.New("unix", *socketFile)
		if err != nil {
			return fmt.Errorf("can't dial mysqlctld: %v", err)
		}
	...
	log.Trace("Checking for updates to my.cnf")
	...
	if bytes.Equal(existing, updated) {
		log.Trace("No changes to my.cnf. Continuing.")
		return nil
	}
	...
	log.Trace("Updated my.cnf. Backup of previous version available in %v", backupPath)
...
func (mysqld *Mysqld) ReinitConfig(ctx context.Context, cnf *Mycnf) error {
	log.Trace("Mysqld.ReinitConfig")

	// Execute as remote action on mysqlctld if requested.
	if *socketFile != "" {
		log.Trace("executing Mysqld.ReinitConfig() remotely via mysqlctld server: %v", *socketFile)
		client, err := mysqlctlclient.New("unix", *socketFile)
		if err != nil {
			return fmt.Errorf("can't dial mysqlctld: %v", err)
		}
...
func (mysqld *Mysqld) createDirs(cnf *Mycnf) error {
	...
	log.Trace("creating directory %s", tabletDir)
...
func (mysqld *Mysqld) createTopDir(cnf *Mycnf, dir string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			log.Trace("creating directory %s", topdir)
	...
	log.Trace("creating directory %s", linkto)
	err = os.MkdirAll(linkto, os.ModePerm)
	if err != nil {
		return err
	}
	log.Trace("creating symlink %s -> %s", source, linkto)
...
func (mysqld *Mysqld) Teardown(ctx context.Context, cnf *Mycnf, force bool) error {
	log.Trace("mysqlctl.Teardown")
	if err := mysqld.Shutdown(ctx, cnf, true); err != nil {
		log.Warn("failed mysqld shutdown: %v", err.Error())
		if !force {
			return err
		}
	}
...
func deleteTopDir(dir string) (removalErr error) {
	...
	if err != nil {
		log.Warn("error deleting dir %v: %v", dir, err.Error())
		removalErr = err
	} else if fi.Mode()&os.ModeSymlink != 0 {
		target, err := filepath.EvalSymlinks(dir)
		if err != nil {
			log.Warn("could not resolve symlink %v: %v", dir, err.Error())
			removalErr = err
		}
		log.Trace("remove data dir (symlinked) %v", target)
		if err = os.RemoveAll(target); err != nil {
			log.Warn("failed removing %v: %v", target, err.Error())
			removalErr = err
		}
	}
	log.Trace("remove data dir %v", dir)
	if err = os.RemoveAll(dir); err != nil {
		log.Warn("failed removing %v: %v", dir, err.Error())
		removalErr = err
	}
	return
}
...
func (mysqld *Mysqld) GetDbaConnection(ctx context.Context) (*dbconnpool.DBConnection, error) {
	params := mysqld.connParams
	// TODO
	//return backend.NewDirectConnection(params.Host+":"+params.Port, params.Uname, params.Pass, params.DbName, params.Charset, mysql2.DefaultCollationID)
	return dbconnpool.NewDBConnection(ctx, params)
}
