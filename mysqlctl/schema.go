...
func (mysqld *Mysqld) executeSchemaCommands(sql string) error {
	return mysqld.executeMysqlScript(mysqld.connParams, strings.NewReader(sql))
}
*/
...
func (mysqld *Mysqld) GetSchema(ctx context.Context, dbName string, tables, excludeTables []string, includeViews bool) (*proto.SchemaDefinition, error) {
	...
	for _, td := range tds {
		...
		wg.Add(1)
		go func(td *proto.TableDefinition) {
			defer wg.Done()
	...
	wg.Wait()
	if len(allErrors) > 0 {
		errStrs := make([]string, 0, len(allErrors))
		for _, oneError := range allErrors {
			errStrs = append(errStrs, oneError.Error())
		}
...
func (mysqld *Mysqld) collectBasicTableData(ctx context.Context, dbName string, tables, excludeTables []string, includeViews bool) ([]*proto.TableDefinition, error) {
	...
	for _, row := range qr.Rows {
		...
		tds = append(tds, &proto.TableDefinition{
			Name:       tableName,
			Type:       tableType,
...
func (mysqld *Mysqld) collectSchema(ctx context.Context, dbName, tableName, tableType string) ([]*proto.Field, []string, string, error) {
...
func (mysqld *Mysqld) GetColumns(ctx context.Context, dbName, table string) ([]*proto.Field, []string, error) {
