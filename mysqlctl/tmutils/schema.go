...
// TableDefinition.
func TableDefinitionGetColumn(td *proto.TableDefinition, name string) (index int, ok bool) {
...
type TableDefinitions []*proto.TableDefinition
...
// (tables), no blacklisted tables (excludeTables) and optionally
// views (includeViews).
func FilterTables(sd *proto.SchemaDefinition, tables, excludeTables []string, includeViews bool) (*proto.SchemaDefinition, error) {
	copy := *sd
	copy.TableDefinitions = make([]*proto.TableDefinition, 0, len(sd.TableDefinitions))
...
func GenerateSchemaVersion(sd *proto.SchemaDefinition) {
	hasher := md5.New()
...
func SchemaDefinitionGetTable(sd *proto.SchemaDefinition, table string) (td *proto.TableDefinition, ok bool) {
	for _, td := range sd.TableDefinitions {
		if td.Name == table {
			return td, true
		}
...
func SchemaDefinitionToSQLStrings(sd *proto.SchemaDefinition) []string {
...
// including views.
func DiffSchema(leftName string, left *proto.SchemaDefinition, rightName string, right *proto.SchemaDefinition, er concurrency.ErrorRecorder) {
...
func DiffSchemaToArray(leftName string, left *proto.SchemaDefinition, rightName string, right *proto.SchemaDefinition) (result []string) {
