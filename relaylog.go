...
func newRelayLog(ctx context.Context, maxItems, maxSize int, stats *Stats) *relayLog {
	rl := &relayLog{
		ctx:      ctx,
		maxItems: maxItems,
		maxSize:  maxSize,
...
func (rl *relayLog) Send(events []*proto.VEvent) error {
	rl.mu.Lock()
	defer rl.mu.Unlock()
	...
	rl.stats.RelayLogLength.Set(int64(len(rl.items)))
	return nil
}

// Fetch returns all existing items in the relay log, and empties the log
func (rl *relayLog) Fetch() ([][]*proto.VEvent, error) {
	rl.mu.Lock()
	defer rl.mu.Unlock()
...
func eventsSize(events []*proto.VEvent) int {
	size := 0
	for _, event := range events {
		if event.Type != proto.VEventType_ROW {
			continue
		}
