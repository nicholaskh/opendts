...
func snapshotConnect(ctx context.Context, cp *mysql.ConnParams) (*snapshotConn, error) {
	mconn, err := mysql.Connect(ctx, cp)
	if err != nil {
		return nil, err
	}
	return &snapshotConn{
...
func (conn *snapshotConn) startSnapshot(ctx context.Context, table string) (gtid string, err error) {
	lockConn, err := mysql.Connect(ctx, conn.cp)
	if err != nil {
		return "", err
	}
	...
	defer func() {
		...
		if err != nil {
			log.Warn("Unlock tables failed: %v", err)
		} else {
			log.Trace("Tables unlocked: %v", table)
		}
		lockConn.Close()
	}()
	...
	log.Trace("Locking table %s for copying", table)
	if _, err := lockConn.ExecuteFetch(fmt.Sprintf("lock tables %s read", tableIdent), 1, false); err != nil {
		log.Trace("Error locking table %s to read", tableIdent)
		return "", err
	}
