...
func (ct *ColumnType) SQLType() proto.Type {
...
func (node *AliasedTableExpr) TableName() (TableName, error) {
	...
	if !ok {
		return TableName{}, errors.New("BUG: the AST has changed. This should not be possible")
	}
...
func ReplaceExpr(root, from, to Expr) Expr {
	...
	if !success {
		log.Warn("Failed to rewrite expression. Rewriter returned a non-expression: " + String(tmp))
		return from
	}
