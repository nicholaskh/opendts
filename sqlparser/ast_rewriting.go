...
func PrepareAST(in Statement, bindVars map[string]*proto.BindVariable, prefix string, parameterize bool, keyspace string) (*RewriteASTResult, error) {
	if parameterize {
...
func RewriteAST(in Statement, keyspace string) (*RewriteASTResult, error) {
	...
	if !ok {
		return nil, fmt.Errorf("statement rewriting returned a non statement: %s", String(out))
	}
...
func (er *expressionRewriter) rewriteAliasedExpr(node *AliasedExpr) (*BindVarNeeds, error) {
	...
	newExpr, ok := tmp.(Expr)
	if !ok {
		return nil, fmt.Errorf("failed to rewrite AST. function expected to return Expr returned a %s", String(tmp))
	}
	node.Expr = newExpr
...
func (er *expressionRewriter) sysVarRewrite(cursor *Cursor, node *ColName) {
	...
	switch lowered {
	case sysvar.Autocommit.Name,
		sysvar.ClientFoundRows.Name,
		sysvar.SkipQueryPlanCache.Name,
		sysvar.SQLSelectLimit.Name,
		sysvar.TransactionMode.Name,
		sysvar.Workload.Name,
		sysvar.DDLStrategy.Name,
		sysvar.SessionUUID.Name,
		sysvar.SessionEnableSystemSettings.Name,
		sysvar.ReadAfterWriteGTID.Name,
		sysvar.ReadAfterWriteTimeOut.Name,
		sysvar.Version.Name,
		sysvar.VersionComment.Name,
		sysvar.SessionTrackGTIDs.Name:
...
func (er *expressionRewriter) funcRewrite(cursor *Cursor, node *FuncExpr) {
	...
	if found {
		...
		if len(node.Exprs) > 0 {
			er.err = fmt.Errorf("Argument to %s() not supported", node.Name.Lowered())
			return
		}
