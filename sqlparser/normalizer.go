...
// treated as distinct.
func Normalize(stmt Statement, bindVars map[string]*proto.BindVariable, prefix string) {
...
func newNormalizer(stmt Statement, bindVars map[string]*proto.BindVariable, prefix string) *normalizer {
	return &normalizer{
		bindVars: bindVars,
		prefix:   prefix,
...
func (nz *normalizer) convertComparison(node *ComparisonExpr) {
	...
	for _, val := range tupleVals {
		...
		bvals.Values = append(bvals.Values, &proto.Value{
			Type:  bval.Type,
			Value: bval.Value,
		})
	}
...
func (nz *normalizer) sqlToBindvar(node SQLNode) *proto.BindVariable {
