...
func (pq *ParsedQuery) GenerateQuery(bindVariables map[string]*proto.BindVariable, extras map[string]Encodable) (string, error) {
...
func (pq *ParsedQuery) Append(buf *strings.Builder, bindVariables map[string]*proto.BindVariable, extras map[string]Encodable) error {
	current := 0
...
func EncodeValue(buf *strings.Builder, value *proto.BindVariable) {
	if value.Type != proto.Type_TUPLE {
...
func FetchBindVar(name string, bindVariables map[string]*proto.BindVariable) (val *proto.BindVariable, isList bool, err error) {
	name = name[1:]
	if name[0] == ':' {
		name = name[1:]
		isList = true
	...
	if isList {
		if supplied.Type != proto.Type_TUPLE {
...
func ParseAndBind(in string, binds ...*proto.BindVariable) (query string, err error) {
