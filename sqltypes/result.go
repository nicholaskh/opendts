...
type Result struct {
	Fields              []*proto.Field `json:"fields"`
...
func FieldsEqual(f1, f2 []*proto.Field) bool {
	if len(f1) != len(f2) {
		return false
	}
	for i, f := range f1 {
		if !proto2.Equal(f, f2[i]) {
			return false
		}
	}
	return true
...
func (result *Result) Repair(fields []*proto.Field) {
...
func (result *Result) Copy() *Result {
	...
	if result.Fields != nil {
		fieldsp := make([]*proto.Field, len(result.Fields))
		fields := make([]proto.Field, len(result.Fields))
...
// MakeRowTrusted converts a *Row to []Value based on the types
...
// why it's justified.
func MakeRowTrusted(fields []*proto.Field, row *proto.Row) []Value {
...
func IncludeFieldsOrDefault(options *proto.ExecuteOptions) proto.ExecuteOptions_IncludedFields {
	if options == nil {
		return proto.ExecuteOptions_TYPE_AND_NAME
	}
...
// individual fields.
func (result *Result) StripMetadata(incl proto.ExecuteOptions_IncludedFields) *Result {
	if incl == proto.ExecuteOptions_ALL || len(result.Fields) == 0 {
		return result
	}
	r := *result
	r.Fields = make([]*proto.Field, len(result.Fields))
	newFieldsArray := make([]proto.Field, len(result.Fields))
	for i, f := range result.Fields {
		r.Fields[i] = &newFieldsArray[i]
		newFieldsArray[i].Type = f.Type
		if incl == proto.ExecuteOptions_TYPE_AND_NAME {
