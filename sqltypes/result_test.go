...
func TestStripMetaData(t *testing.T) {
	testcases := []struct {
		...
		in             *Result
		expected       *Result
		includedFields proto.ExecuteOptions_IncludedFields
	}{{
		name:     "no fields",
		in:       &Result{},
		expected: &Result{},
		...
		name:           "no name",
		includedFields: proto.ExecuteOptions_TYPE_ONLY,
		in: &Result{
			Fields: []*proto.Field{{
				Type: Int64,
			}, {
		...
		includedFields: proto.ExecuteOptions_ALL,
		in: &Result{
			Fields: []*proto.Field{{
				Name:         "field1",
				Table:        "table1",
	...
	for _, tcase := range testcases {
		...
		if len(tcase.in.Fields) > 0 {
			...
			if out.Fields[0] == inCopy.Fields[0] && tcase.includedFields != proto.ExecuteOptions_ALL {
