...
// MakeTestFields builds a []*proto.Field for testing.
//   fields := sqltypes.MakeTestFields(
//     "a|b",
//     "int64|varchar",
//   )
// The field types are as defined in proto and are case
// insensitive. Column delimiters must be used only to sepearate
// strings and not at the beginning or the end.
func MakeTestFields(names, types string) []*proto.Field {
	n := split(names)
	t := split(types)
	...
	for i := range n {
		fields = append(fields, &proto.Field{
			Name: n[i],
			Type: proto.Type(proto.Type_value[strings.ToUpper(t[i])]),
		})
	}
	return fields
}
...
func MakeTestResult(fields []*proto.Field, rows ...string) *Result {
	result := &Result{
		Fields: fields,
	}
	if len(rows) > 0 {
...
func MakeTestStreamingResults(fields []*proto.Field, rows ...string) []*Result {
	var results []*Result
...
// TestBindVariable makes a *proto.BindVariable from
// an interface{}.It panics on invalid input.
// This function should only be used for testing.
func TestBindVariable(v interface{}) *proto.BindVariable {
	if v == nil {
...
// This function should only be used for testing.
func TestValue(typ proto.Type, val string) Value {
