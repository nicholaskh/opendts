...
func (t *Timings) RecordDelay(name string, delay int64) {
	t.Add(name, time.Duration(delay) * time.Millisecond)
}
...
func (mt *MultiTimings) RecordDelay(names []string, delay int64) {
	if len(names) != len(mt.labels) {
		panic("MultiTimings: wrong number of values in Record")
	}
	mt.Timings.RecordDelay(safeJoinLabels(names), delay)
}
